  ...
  **/
  var XHR = function(async) {
    ...
    **/
    ...
    _blank_func = function() {},

    _init_event = function() {
      return {
        'ready': _blank_func, // when connected to server 
        'error': _blank_func, // when error
        'success': _blank_func, // when server status code == 200
        'fail': _blank_func // when server status code != 200
      };
    }, 
    _method = '',
    _url = '',
    ...
    **/
    on = function(event, callback) {
      if ('ready-error-success-fail'.split('-').indexOf(event) >= 0 && _typeof(callback) == 'function') 
        _events[event] = callback;
    },

    // onreadystatechange callback function 
    _on_request_callback = function() {
    ...
    **/
    ...
    _request = function() {
      ...
      _xhr.onreadystatechange = _on_request_callback;
      _xhr.send(_body);
      return _this;
    },
